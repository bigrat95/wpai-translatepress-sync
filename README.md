...
**Important:** The plugin automatically injects translated variation descriptions into WooCommerce's frontend JSON data (`data-product_variations`). This is necessary because TranslatePress cannot translate strings embedded inside JSON HTML attributes. Without this, selecting a variation via JavaScript would replace the server-translated description with the untranslated English version. No theme modifications are required — this works with any WooCommerce-compatible theme.
...
**A:** Yes! Full WooCommerce support including product categories, attributes, and variation descriptions.

### Q: Why do my variation descriptions show in English even though translations exist?

**A:** WooCommerce embeds variation data as JSON inside an HTML attribute (`data-product_variations`). TranslatePress cannot translate strings inside this JSON. The plugin handles this automatically (since v3.5.0) by injecting the translated variation description directly into the JSON data via the `woocommerce_available_variation` filter. Make sure `_trp_variation_desc_[lang]` meta exists on your variations.
...
### 3.5.0
- **NEW:** WooCommerce frontend variation description translation injection
- TranslatePress cannot translate strings inside WooCommerce's `data-product_variations` JSON attribute — the plugin now injects translated descriptions directly via the `woocommerce_available_variation` filter
- Works with any WooCommerce-compatible theme, no theme modifications required
- Uses the `$TRP_LANGUAGE` global to detect the current language
