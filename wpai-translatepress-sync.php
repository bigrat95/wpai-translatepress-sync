...
class WPAI_TranslatePress_Sync {
    ...
    public function __construct() {
        ...
        // Inject translated variation descriptions into WooCommerce JSON variation data
        // TranslatePress cannot translate strings inside the data-product_variations JSON attribute,
        // so we replace variation_description with the translated version at the PHP level
        add_filter( 'woocommerce_available_variation', array( $this, 'inject_variation_description_translation' ), 20, 3 );
    }
    ...
    /**
     * Inject translated variation description into WooCommerce's available_variations JSON data
     * 
     * TranslatePress translates server-rendered HTML text but cannot translate strings
     * inside WooCommerce's data-product_variations JSON attribute. When JavaScript fires
     * the found_variation event, it replaces the translated text with the untranslated
     * English from the JSON. This method injects the translation directly into the JSON
     * so the frontend receives the correct language.
     * 
     ...
     */
    public function inject_variation_description_translation( $data, $product, $variation ) {
        global $TRP_LANGUAGE;
        ...
        $translated_desc = get_post_meta( $variation_id, $this->variation_desc_prefix . $TRP_LANGUAGE, true );
        
        if ( ! empty( $translated_desc ) ) {
            $data['variation_description'] = wc_format_content( $translated_desc );
        }
